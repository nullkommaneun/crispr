...
const WIN_MIN=0.70, WIN_MAX=2.00, EPS=0.10, HUNGER_GATE=0.45, EARLY_DE_ABS=2.0;
const K_DIST=0.60, R_PAIR=10;
...
function featuresForOption(cell,ctx,option){
  ...
  const metZ = z(g.MET); // positives Feature, Gewicht in w negativ
...
function tracePush(m,cell,ctx,shaped,winner,scoreDelta){
  if(!TRACE_ON) return;
  trace.push({ dur:m.tAccum,id:cell.id,name:cell.name,st:cell.stammId,opt:m.chosen,forced:m.forced,p:0,
    e0:m.e0,e1:cell.energy,dE:shaped,dFoodNow:ctx.foodDist??null,dMate:ctx.mateDist??null,haz:ctx.hazard??0,win:winner,scoreDelta });
