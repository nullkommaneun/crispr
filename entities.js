// entities.js – Welt, Zellen, Nahrung, Verhalten (mit Spatial Grid)
...
// --- Spatial Grid -----------------------------------------------------------
const GRID = {
  size: 48,    // Zellenweite in px
  cols: 0, rows: 0,
  foodB: [],   // Array<Array<food>>
  cellB: []    // Array<Array<cell>>
};
function gridResize(){
  ...
  // Foods reindizieren
  for(const f of foods) addFoodToGrid(f);
}
...
function removeFoodFromGrid(fid){
  for(const bucket of GRID.foodB){
    const idx = bucket.findIndex(o=>o.id===fid);
    if(idx!==-1){ bucket.splice(idx,1); return; }
  }
}
...
export function createCell(params={}){
  ...
  const c={ id, name:params.name||`Zelle #${id}`, stammId, sex,
            ...
            genes, energy:params.energy??22, age:0, dead:false, parents, bornAt:performance.now()/1000, lastMateAt:-999 };
...
export function createFood(params={}){ const f={ id:params.id??nextFoodId++, x:params.x??Math.random()*WORLD.width, y:params.y??Math.random()*WORLD.height, value:params.value??10 }; foods.push(f); addFoodToGrid(f); Events.emit(EVT.FOOD_SPAWN,{id:f.id}); return f; }
...
// --- Export/Import (intern belassen) ---------------------------------------
export function exportState(){
  return JSON.stringify({ WORLD,nextCellId,nextFoodId,nextStammId,
...
function ancUp(id,depth){ const map=new Map(); const q=[{id,d:0}];
  while(q.length){ const {id:cur,d}=q.shift(); const p=pedigree.get(cur); if(!p) continue;
  ...
  for(const c of cells){
    ...
    for(const f of [...neighborFoods(c.x,c.y)]){
      ...
      if(dx*dx+dy*dy <= (c.radius+3)*(c.radius+3)){
        ...
        const idx = foods.findIndex(ff=>ff.id===f.id);
...
function foodSpawner(dt){
  ...
  if(n>0){
    foodAcc -= n;
    for(let i=0;i<n;i++){ if(foods.length < WORLD.maxFood) createFood(); }
  }
}
...
export function updateWorld(dt){
  ...
  // Cell-Grid pro Frame neu
  rebuildCellGrid(alive);

  for(const c of alive) updateCellBehavior(c, alive, dt);
  eatPhase();

  // Paarung mit Nachbarsuche (Grid)
  const neighborQuery = (cell)=> neighborCells(cell.x, cell.y);
  evaluateMatingPairs(
    alive,
    (params)=>createCell(params),
    { mutationRate: WORLD.mutationRate, relatednessFn: relatedness, neighborQuery }
  );
