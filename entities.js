// entities.js – Welt, Zellen, Nahrung, Verhalten (mit Spatial Grid + Scheduler)
...
function* neighborFoods(x,y){ const gx=Math.max(0,Math.min(GRID.cols-1,(x/GRID.size|0))); const gy=Math.max(0,Math.min(GRID.rows-1,(y/GRID.size|0)));
  for(let yy=gy-1;yy<=gy+1;yy++){ if(yy<0||yy>=GRID.rows) continue;
    for(let xx=gx-1;xx<=gx+1;xx++){ if(xx<0||xx>=GRID.cols) continue; yield* GRID.foodB[yy*GRID.cols+xx]; } } }
function* neighborCells(x,y){ const gx=Math.max(0,Math.min(GRID.cols-1,(x/GRID.size|0))); const gy=Math.max(0,Math.min(GRID.rows-1,(y/GRID.size|0)));
  for(let yy=gy-1;yy<=gy+1;yy++){ if(yy<0||yy>=GRID.rows) continue;
    for(let xx=gx-1;xx<=gx+1;xx++){ if(xx<0||xx>=GRID.cols) continue; yield* GRID.cellB[yy*GRID.cols+xx]; } } }

// ---------- Weltzeit & Scheduler ----------
let worldTime = 0;
const scheduled = []; // {due:number, fn:Function}
/** Planer: führt fn nach delaySec Simulationssekunden aus (wirkt mit Timescale) */
...
// ---------- Welt-API ----------
export function setWorldSize(w,h){ WORLD.width=Math.max(50,w|0); WORLD.height=Math.max(50,h|0); gridResize(); }
  ...
  if(!params.noSplit && parents?.motherId){
    ...
    if(mother){
      ...
      if(sum>=thresh || mx>=SPEC.split.max || Math.random()<SPEC.split.randomProb){
        stammId=newStammId();
        Events.emit(EVT.TIP,{label:'🧬 Abspaltung', text:`Neuer Stamm ${stammId}: starke Genabweichung.`});
      }
    }
  }
  const sex=params.sex??(Math.random()<0.5?'m':'f'); const genes=params.genes?{...params.genes}:createGenome();
...
export function createFood(params={}){ const f={ id:params.id??nextFoodId++, x:params.x??Math.random()*WORLD.width, y:params.y??Math.random()*WORLD.height, value:params.value??10 };
  foods.push(f); addFoodToGrid(f); Events.emit(EVT.FOOD_SPAWN,{id:f.id}); return f; }
...
export function exportState(){ return JSON.stringify({ WORLD,nextCellId,nextFoodId,nextStammId,
    cells:cells.filter(c=>!c.dead).map(c=>({ id:c.id,name:c.name,stammId:c.stammId,sex:c.sex,x:c.x,y:c.y,vx:c.vx,vy:c.vy,genes:c.genes,radius:c.radius,energy:c.energy,age:c.age,parents:c.parents,bornAt:c.bornAt,lastMateAt:c.lastMateAt })),
    foods, foundersIds },null,2); }
...
function updateCellBehavior(c, alive, dt){
  ...
  let dVX=0, dVY=0;
  if(c.target){ const dx=c.target.x-c.x, dy=c.target.y-c.y; const d=Math.max(1,Math.hypot(dx,dy)); const sp=c.derived.speedMax; dVX=(dx/d)*sp; dVY=(dy/d)*sp; }
  else{ c.wanderAng=(c.wanderAng??Math.random()*Math.PI*2)+(Math.random()*2-1)*0.3*dt; const sp=c.derived.speedMax*0.6; dVX=Math.cos(c.wanderAng)*sp; dVY=Math.sin(c.wanderAng)*sp; }
...
function deathPhase(){
  ...
  for(const c of cells){
    if(c.dead) continue;
    if(c.energy<=0){ c.dead=true; lastMinuteHungerDeaths.push(now); Events.emit(EVT.DEATH,{id:c.id,stammId:c.stammId,reason:'hunger'}); }
  }
...
function foodSpawner(dt){
  ...
  if(n>0){ foodAcc-=n; for(let i=0;i<n;i++){ if(foods.length<WORLD.maxFood) createFood(); } }
}

// ---------- Hauptupdate ----------
export function updateWorld(dt){
  worldTime += dt;   // wichtig für den Scheduler!
  runScheduler();
  ...
  evaluateMatingPairs(alive, (params)=>createCell(params), { mutationRate: WORLD.mutationRate, relatednessFn: relatedness, neighborQuery: (cell)=>neighborCells(cell.x,cell.y) });
