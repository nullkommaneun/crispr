...
export function createAdamAndEve(){
  ...
  const A=createCell({ name:"Adam", sex:"M", stammId:1, genome:gA, pos:{x:cx-gap,y:cy},
                       energy: capEnergy({genome:gA})*0.85 });
  const E=createCell({ name:"Eva",  sex:"F", stammId:2, genome:gE, pos:{x:cx+gap,y:cy},
                       energy: capEnergy({genome:gE})*0.85 });
...
function makeChild(A,E,k){
  const g={ TEM:mixGene(A.genome.TEM,E.genome.TEM),
  ...
  return {
    id:nextCellId++, name:`C${1000+k}`, sex:(Math.random()<0.5?"M":"F"),
    stammId:st, color:pickColorByStamm(st),
    pos:{x:W*0.5+Math.cos(ang)*r, y:H*0.5+Math.sin(ang)*r},
    vel:{x:0,y:0}, energy:cap*0.75, age:0, cooldown:0, genome:g,
    wander:{vx:0,vy:0}
  };
}

/** Placeholder (wir lesen env live) */
...
function nearestFoodCenter(c, sense){
  ...
  const cx=take.reduce((s,f)=>s+f.x,0)/take.length, cy=take.reduce((s,f)=>s+f.y,0)/take.length;
...
function chooseMate(c, sense, arr){
  ...
  for(const o of arr){
    ...
    const dx=o.pos.x-c.pos.x, dy=o.pos.y-c.pos.y; const d2=dx*dx+dy*dy; if(d2>sense*sense) continue;
...
/* ===== Hauptschritt (dt-basiert für Drives) ===== */
export function step(dt, env /*t ungenutzt*/){
  ...
  for(let i=cells.length-1;i>=0;i--){
    ...
    if(env.fence?.enabled && dEdge<env.fence.range){
      ...
      c.vel.y += fyz*(env.fence.impulse||0);
    }

    // Lernen (dt-basiertes Fenster)
...
function quickSeparation(c,arr,r){
