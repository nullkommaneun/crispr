// advisor.js – Heuristik / optional TF.js Modell. Modussteuerung im Editor.
...
export async function tryLoadTF(){
  ...
  return new Promise((resolve)=>{
    const ex = document.querySelector('script[data-crispr-tf]'); if(ex){ state.libReady=!!window.tf; return resolve(state.libReady); }
...
export async function cycleAdvisorMode(defaultModelUrl){
  if(!state.enabled){ setEnabled(true); await tryLoadTF(); state.useModel=false; return 'heuristic'; }
  if(state.model){ state.useModel=!state.useModel; return state.useModel?'model':'heuristic'; }
  if(defaultModelUrl){ const m=await loadModelFromUrl(defaultModelUrl).catch(()=>null); if(m){ state.useModel=true; return 'model'; } }
...
export function updateAdvisor(nowSec){
  ...
  if(state.metrics.hungerDeaths >= 5){ tip('Tipp','Viele Zellen verhungern. Erhöhe die Nahrung.'); state.metrics.hungerDeaths=0; }
  if(state.metrics.deaths > state.metrics.births*1.5){ tip('Tipp','Sterben deutlich mehr Zellen als geboren werden. Prüfe Mutationsrate und Nahrung.'); state.metrics.deaths=0; state.metrics.births=0; }
}