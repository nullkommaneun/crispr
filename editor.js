// editor.js – CRISPR-Editor: 5 Traits inkl. MET; robust wie zuvor.
...
export function initEditor(){
  ...
  // Stepper initialisieren – alle vorhandenen .traitRow werden genutzt
  ...
  function setFromGenome(g){
    ...
    for(const row of document.querySelectorAll('.traitRow')){
      const trait=row.dataset.trait; const out=row.querySelector('.val');
      out.textContent = String(current[trait] ?? 5);
    }
  }

  const formatScore = (p)=> p==null ? '–' : `${Math.round(p*100)}<small>%</small>`;
  ...
  function refreshList(){
    ...
    for(const c of alive){
      ...
      card.innerHTML=`<span class="id">${c.name} <small class="mono">• Stamm ${c.stammId}</small></span><span class="score">${formatScore(p)}</span>`;
      card.addEventListener('click', ()=>{
        selectedId=c.id; setFromGenome(c.genes);
        for(const el of list.querySelectorAll('.cellCard')) el.classList.remove('active'); card.classList.add('active');
      });
      list.appendChild(card);
    }
  }
  refreshList(); Events.on(EVT.BIRTH, refreshList); Events.on(EVT.DEATH, refreshList); Events.on(EVT.STATUS, refreshAdvisorUI);

  form.addEventListener('submit',(e)=>{
    e.preventDefault();
    const genome=createGenome({...current});
    const stamm=Entities.newStammId?.();
    const create = Entities.createCell;
    if(typeof create!=='function') return;
    const c=create({ x:Math.random()*800, y:Math.random()*520, genes:genome, stammId: stamm??undefined, energy:22 });
  ...
  btnLoad.addEventListener('click', async ()=>{
    ...
    try{ await loadModelFromUrl(url); setEnabled(true); setUseModel(true); refreshAdvisorUI(); refreshList(); Events.emit(EVT.STATUS,{source:'editor',text:'KI-Modell geladen'});}catch{}
  });
