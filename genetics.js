...
/**
 * Rekombination: mittelt Eltern, fügt Variation/Mutation hinzu.
 * inbreeding (0..1) erhöht Drift zu Extremen leicht.
 */
 ...
 * TEM+, EFF+++, SCH+, GRO± (leicht), MET optimum um 5 (Abweichung −).
 */
export function survivalScore(g){
  const n = x => (x-5)/4;              // -1..1
  const metPenalty = -Math.abs(n(g.MET)); // Optimum bei 5
  ...
  let s = 50
    ...
    + 35*0.10*(metPenalty); // MET: Abweichung kostet
